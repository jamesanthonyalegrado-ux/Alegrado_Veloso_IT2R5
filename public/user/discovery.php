...
// Get all books for full discovery
$sqlAllBooks = "SELECT uuid, title, author, publisher, yearPublished, category_id, description FROM books ORDER BY title ASC";
$resultAllBooks = $conn->query($sqlAllBooks);
$allBooks = [];
if ($resultAllBooks && $resultAllBooks->num_rows > 0) {
...
if ($searchQuery !== '') {
    ...
    $sqlSearch = "SELECT uuid, title, author, publisher, yearPublished, category_id, description FROM books WHERE title LIKE ? OR author LIKE ? OR publisher LIKE ? OR description LIKE ? ORDER BY title ASC LIMIT 20";
    ...
    <div class="section mb-5">
        ...
        <div class="books-grid">
            <?php if (!empty($searchResults)): ?>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <div class="text-muted">No books matched your search. Try a different title, author, or keyword.</div>
            <?php endif; ?>
        </div>
    </div>
    <?php endif; ?>
